...
Program — the top-level configuration for a customer's payouts relationship with PayQuicker. Carries the operating currency, the sponsoring bank, the program type ([Hosted Portal vs API Gateway](page:concepts/program-types)), and the company funding accounts associated with the program. Programs are read-only via the API; they are configured by PayQuicker at onboarding. Token prefix `prog-`. See [Programs](page:resources/programs).
