...
A Hosted Portal [Invitation](page:resources/invitations) sent to a prospective payee, returned with HATEOAS navigation links. The `status` field tracks the invitation lifecycle, and `registrationStatus` tracks the payee's progress through onboarding (see [User Status Types](page:additional-api-information/user-status-types)).
...
```ruby
invitation_object = InvitationObject.new(
