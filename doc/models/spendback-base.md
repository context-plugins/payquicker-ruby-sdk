...
Spendback quote — pulls funds from a payee's wallet back to the program's funding account, typically when the payee uses their balance to purchase goods or services from the payer. Created via `POST /transfers` with `transferType: SPENDBACK`. Token prefix `spnd-`. Follows the standard quote / accept lifecycle. See [Transfers](page:resources/transfers).
...
```ruby
spendback_base = SpendbackBase.new(
