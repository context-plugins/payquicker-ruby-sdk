...
# Payment Retraction Search Filter Item

A single filter row inside a `POST /transfers/search` request body's `filters` array. The `field` value comes from `PaymentRetractionSearchFilterField`; valid `comparison` values depend on the field's data type (see [Filtering & Sorting](page:concepts/filtering-sorting)). See [`Transfer Search Fields`](page:additional-api-information/transfer-search-fields#transfertype-retraction) for the complete catalog of supported field names.

*This model accepts additional fields of type Object.*

## Structure

`PaymentRetractionSearchFilterItem`

## Fields

| Name | Type | Tags | Description |
|  --- | --- | --- | --- |
| `field` | `String` | Required | Filterable fields for payment retraction search. |
...
```ruby
payment_retraction_search_filter_item = PaymentRetractionSearchFilterItem.new(
  field: 'CREATED_AT',
