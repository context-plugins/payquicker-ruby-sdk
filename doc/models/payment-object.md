...
A completed or in-progress payment record, returned with HATEOAS navigation links. Created via [`POST /transfers`](page:resources/transfers) with `transferType: PAYMENT`. See [Transfers](page:resources/transfers) and [Payment Search Fields](page:additional-api-information/transfer-search-fields#transfertype-payment).
...
```ruby
payment_object = PaymentObject.new(
