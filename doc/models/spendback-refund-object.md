...
A spendback-refund record — return of a previously debited spendback to the payee, returned with HATEOAS navigation links. Created via [`POST /transfers`](page:resources/transfers) with `transferType: REFUND`. See [Transfers](page:resources/transfers).
...
```ruby
spendback_refund_object = SpendbackRefundObject.new(
