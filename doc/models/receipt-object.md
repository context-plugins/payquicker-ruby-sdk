...
A [Receipt](page:resources/receipts) — the immutable record of a completed transaction (payment, spendback, transfer, fee, retraction, etc.), returned with HATEOAS navigation links. Receipt types are listed at [Receipt Types](page:additional-api-information/receipt-types); status lifecycle at [Receipt Status Types](page:additional-api-information/receipt-status-types).
...
```ruby
receipt_object = ReceiptObject.new(
