...
# Payment Job Item Base

A single payment row within a [Payment Job](page:resources/payment-jobs), combining payment properties with job-specific context (parent job association, row index, validation outcome). One row corresponds to one CSV line in the original batch submission.
...
```ruby
payment_job_item_base = PaymentJobItemBase.new(
