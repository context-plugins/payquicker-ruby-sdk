...
An identity-verification check result attached to a [User](page:resources/users) — captures the outcome of a single KYC verification step (document validation, sanctions screening, etc.). See [KYC Enums](page:additional-api-information/kyc-enums) for the result-type catalog.
...
```ruby
idv_check_base = IdvCheckBase.new(
