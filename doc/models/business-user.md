...
| `first_name` | `String` | Required | Contact person's first name for the business. |
| `last_name` | `String` | Required | Contact person's last name for the business. |
| `date_of_birth` | `Date` | Required | Contact person's date of birth for the business. |
| `contact_role` | `String` | Required | Role of a business-user's contact — for example, `OWNER`, `MANAGER`, `OFFICER`. Used on `BUSINESS` user types to identify the natural person responsible for the business relationship. See [KYC Enums](page:additional-api-information/kyc-enums) for the full value list.<br><br>**Constraints**: *Maximum Length*: `20` |
| `business_name` | `String` | Required | Registered business name. |
...
| `addresses` | [`Array[AddressObject]`](../../doc/models/address-object.md) | Required | BUSINESS requires BUSINESS and MAILING. |
| `phone_numbers` | [`Array[PhoneNumberObject]`](../../doc/models/phone-number-object.md) | Required | PRIMARY is required. |
| `user_type` | [`UserType3`](../../doc/models/user-type-3.md) | Required | Discriminator identifying this as a business user. |
