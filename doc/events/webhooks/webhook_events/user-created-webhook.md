...
A new user has been created. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserCreatedWebhookRequest](../../../../doc/models/user-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserCreatedWebhookRequest) && event.event_type == 'USERS.CREATED'
      puts 'UserCreatedWebhook received'
      # TODO: Add usercreatedwebhook handling
