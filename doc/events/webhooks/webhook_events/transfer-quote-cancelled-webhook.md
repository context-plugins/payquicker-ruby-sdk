...
# Transfer Quote Cancelled Webhook

A transfer quote has been cancelled. See [Transfers — Webhook Events](page:webhook-events/webhookevents-transfers) for details.
...
This event's request payload is of type [TransferQuoteCancelledWebhookRequest](../../../../doc/models/transfer-quote-cancelled-webhook-request.md).
...
```ruby
# Implementation example of handling the `TransferQuoteCancelledWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(TransferQuoteCancelledWebhookRequest) && event.event_type == 'TRANSFERQUOTES.CANCELLED'
      puts 'TransferQuoteCancelledWebhook received'
      # TODO: Add transferquotecancelledwebhook handling
