...
# Receipt Returned Webhook

Receipt status changed to returned. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptReturnedWebhookRequest](../../../../doc/models/receipt-returned-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptReturnedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptReturnedWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.STATUS.RETURNED'
      puts 'ReceiptReturnedWebhook received'
      # TODO: Add receiptreturnedwebhook handling
