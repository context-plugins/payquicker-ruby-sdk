...
# Prepaid Card Closed Stolen Webhook

Card status changed to closed (stolen). See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardClosedStolenWebhookRequest](../../../../doc/models/prepaid-card-closed-stolen-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardClosedStolenWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardClosedStolenWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.CLOSEDSTOLEN'
      puts 'PrepaidCardClosedStolenWebhook received'
      # TODO: Add prepaidcardclosedstolenwebhook handling
