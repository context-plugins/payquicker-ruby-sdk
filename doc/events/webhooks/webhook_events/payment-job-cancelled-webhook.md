...
# Payment Job Cancelled Webhook

A batch payment job has been cancelled. See [Batch Payments — Webhook Events](page:webhook-events/webhookevents-batch-payments) for details.
...
This event's request payload is of type [PaymentJobCancelledWebhookRequest](../../../../doc/models/payment-job-cancelled-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentJobCancelledWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentJobCancelledWebhookRequest) && event.event_type == 'PAYMENTJOBS.CANCELLED'
      puts 'PaymentJobCancelledWebhook received'
      # TODO: Add paymentjobcancelledwebhook handling
