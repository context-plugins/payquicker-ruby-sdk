...
# Spendback Refund Voided Webhook

A spendback refund quote has been voided. See [Spendback Refunds — Webhook Events](page:webhook-events/webhookevents-spendback-refunds) for details.
...
This event's request payload is of type [SpendbackRefundVoidedWebhookRequest](../../../../doc/models/spendback-refund-voided-webhook-request.md).
...
```ruby
# Implementation example of handling the `SpendbackRefundVoidedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(SpendbackRefundVoidedWebhookRequest) && event.event_type == 'SPENDBACKREFUNDQUOTES.VOIDED'
      puts 'SpendbackRefundVoidedWebhook received'
      # TODO: Add spendbackrefundvoidedwebhook handling
