...
# Bank Account Modified Webhook

Bank account details have been modified. See [Bank Accounts — Webhook Events](page:webhook-events/webhookevents-bank-accounts) for details.
...
This event's request payload is of type [BankAccountModifiedWebhookRequest](../../../../doc/models/bank-account-modified-webhook-request.md).
...
```ruby
# Implementation example of handling the `BankAccountModifiedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(BankAccountModifiedWebhookRequest) && event.event_type == 'BANKACCOUNTS.UPDATED.DETAILS.MODIFIED'
      puts 'BankAccountModifiedWebhook received'
      # TODO: Add bankaccountmodifiedwebhook handling
