...
# Invitation Used Webhook

An invitation has been used by the payee. See [Invitations — Webhook Events](page:webhook-events/webhookevents-invitations) for details.
...
This event's request payload is of type [InvitationUsedWebhookRequest](../../../../doc/models/invitation-used-webhook-request.md).
...
```ruby
# Implementation example of handling the `InvitationUsedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(InvitationUsedWebhookRequest) && event.event_type == 'INVITATIONS.USED'
      puts 'InvitationUsedWebhook received'
      # TODO: Add invitationusedwebhook handling
