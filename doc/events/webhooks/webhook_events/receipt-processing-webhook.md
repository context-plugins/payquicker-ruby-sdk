...
Receipt is being processed. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptProcessingWebhookRequest](../../../../doc/models/receipt-processing-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptProcessingWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptProcessingWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.STATUS.PROCESSING'
      puts 'ReceiptProcessingWebhook received'
      # TODO: Add receiptprocessingwebhook handling
