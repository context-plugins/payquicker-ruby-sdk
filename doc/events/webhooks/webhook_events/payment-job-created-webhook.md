...
# Payment Job Created Webhook

A new batch payment job has been created. See [Batch Payments — Webhook Events](page:webhook-events/webhookevents-batch-payments) for details.
...
This event's request payload is of type [PaymentJobCreatedWebhookRequest](../../../../doc/models/payment-job-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentJobCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentJobCreatedWebhookRequest) && event.event_type == 'PAYMENTJOBS.CREATED'
      puts 'PaymentJobCreatedWebhook received'
      # TODO: Add paymentjobcreatedwebhook handling
