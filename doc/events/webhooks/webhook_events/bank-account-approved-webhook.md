...
# Bank Account Approved Webhook

Bank account status changed to approved. See [Bank Accounts — Webhook Events](page:webhook-events/webhookevents-bank-accounts) for details.
...
This event's request payload is of type [BankAccountApprovedWebhookRequest](../../../../doc/models/bank-account-approved-webhook-request.md).
...
```ruby
# Implementation example of handling the `BankAccountApprovedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(BankAccountApprovedWebhookRequest) && event.event_type == 'BANKACCOUNTS.UPDATED.STATUS.APPROVED'
