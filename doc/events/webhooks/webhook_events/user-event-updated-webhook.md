...
# User Event Updated Webhook

A user event has been updated. See [User Events — Webhook Events](page:webhook-events/webhookevents-user-events) for details.
...
This event's request payload is of type [UserEventUpdatedWebhookRequest](../../../../doc/models/user-event-updated-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserEventUpdatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserEventUpdatedWebhookRequest) && event.event_type == 'USEREVENTS.UPDATED'
      puts 'UserEventUpdatedWebhook received'
      # TODO: Add usereventupdatedwebhook handling
