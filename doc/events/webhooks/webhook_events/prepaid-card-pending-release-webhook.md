...
# Prepaid Card Pending Release Webhook

Card status changed to pending release. See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardPendingReleaseWebhookRequest](../../../../doc/models/prepaid-card-pending-release-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardPendingReleaseWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardPendingReleaseWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.PENDINGRELEASE'
      puts 'PrepaidCardPendingReleaseWebhook received'
      # TODO: Add prepaidcardpendingreleasewebhook handling
