...
# User Idv Passed Webhook

User identity verification has passed. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserIdvPassedWebhookRequest](../../../../doc/models/user-idv-passed-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserIdvPassedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserIdvPassedWebhookRequest) && event.event_type == 'USERS.UPDATED.IDV.PASSED'
      puts 'UserIdvPassedWebhook received'
      # TODO: Add useridvpassedwebhook handling
