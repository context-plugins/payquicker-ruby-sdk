...
# Invitation Job Updated Webhook

A batch invitation job has been updated. See [Batch Invitations — Webhook Events](page:webhook-events/webhookevents-batch-invitations) for details.
...
This event's request payload is of type [InvitationJobUpdatedWebhookRequest](../../../../doc/models/invitation-job-updated-webhook-request.md).
...
```ruby
# Implementation example of handling the `InvitationJobUpdatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(InvitationJobUpdatedWebhookRequest) && event.event_type == 'INVITATIONJOBS.UPDATED'
      puts 'InvitationJobUpdatedWebhook received'
      # TODO: Add invitationjobupdatedwebhook handling
