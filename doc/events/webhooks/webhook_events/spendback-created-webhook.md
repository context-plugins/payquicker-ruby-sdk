...
# Spendback Created Webhook

A new spendback quote has been created. See [Spendbacks — Webhook Events](page:webhook-events/webhookevents-spendbacks) for details.
...
This event's request payload is of type [SpendbackCreatedWebhookRequest](../../../../doc/models/spendback-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `SpendbackCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(SpendbackCreatedWebhookRequest) && event.event_type == 'SPENDBACKQUOTES.CREATED'
      puts 'SpendbackCreatedWebhook received'
      # TODO: Add spendbackcreatedwebhook handling
