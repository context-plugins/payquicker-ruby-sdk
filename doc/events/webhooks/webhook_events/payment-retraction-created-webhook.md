...
# Payment Retraction Created Webhook

A new payment retraction quote has been created. See [Payment Retractions — Webhook Events](page:webhook-events/webhookevents-payment-retractions) for details.
...
This event's request payload is of type [PaymentRetractionCreatedWebhookRequest](../../../../doc/models/payment-retraction-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentRetractionCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentRetractionCreatedWebhookRequest) && event.event_type == 'PAYMENTRETRACTIONQUOTES.CREATED'
      puts 'PaymentRetractionCreatedWebhook received'
      # TODO: Add paymentretractioncreatedwebhook handling
