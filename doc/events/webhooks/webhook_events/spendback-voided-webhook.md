...
# Spendback Voided Webhook

A spendback quote has been voided. See [Spendbacks — Webhook Events](page:webhook-events/webhookevents-spendbacks) for details.
...
This event's request payload is of type [SpendbackVoidedWebhookRequest](../../../../doc/models/spendback-voided-webhook-request.md).
...
```ruby
# Implementation example of handling the `SpendbackVoidedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(SpendbackVoidedWebhookRequest) && event.event_type == 'SPENDBACKQUOTES.VOIDED'
      puts 'SpendbackVoidedWebhook received'
      # TODO: Add spendbackvoidedwebhook handling
