...
# Receipt Scheduled Webhook

Receipt status changed to scheduled. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptScheduledWebhookRequest](../../../../doc/models/receipt-scheduled-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptScheduledWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptScheduledWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.STATUS.SCHEDULED'
      puts 'ReceiptScheduledWebhook received'
      # TODO: Add receiptscheduledwebhook handling
