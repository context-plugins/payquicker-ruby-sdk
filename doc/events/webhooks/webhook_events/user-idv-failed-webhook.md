...
# User Idv Failed Webhook

User identity verification has failed. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserIdvFailedWebhookRequest](../../../../doc/models/user-idv-failed-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserIdvFailedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserIdvFailedWebhookRequest) && event.event_type == 'USERS.UPDATED.IDV.FAILED'
      puts 'UserIdvFailedWebhook received'
      # TODO: Add useridvfailedwebhook handling
