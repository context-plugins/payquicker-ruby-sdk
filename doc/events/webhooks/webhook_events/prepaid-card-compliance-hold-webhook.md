...
# Prepaid Card Compliance Hold Webhook

Card placed on compliance hold. See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardComplianceHoldWebhookRequest](../../../../doc/models/prepaid-card-compliance-hold-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardComplianceHoldWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardComplianceHoldWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.COMPLIANCEHOLD'
      puts 'PrepaidCardComplianceHoldWebhook received'
      # TODO: Add prepaidcardcomplianceholdwebhook handling
