...
# User Details Updated Webhook

User details have been updated. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserDetailsUpdatedWebhookRequest](../../../../doc/models/user-details-updated-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserDetailsUpdatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserDetailsUpdatedWebhookRequest) && event.event_type == 'USERS.UPDATED.DETAILS'
      puts 'UserDetailsUpdatedWebhook received'
      # TODO: Add userdetailsupdatedwebhook handling
