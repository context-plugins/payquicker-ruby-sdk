...
# Prepaid Card Active Webhook

Card status changed to active. See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardActiveWebhookRequest](../../../../doc/models/prepaid-card-active-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardActiveWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardActiveWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.ACTIVE'
      puts 'PrepaidCardActiveWebhook received'
      # TODO: Add prepaidcardactivewebhook handling
