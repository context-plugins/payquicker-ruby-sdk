...
# Payment Retraction Voided Webhook

A payment retraction quote has been voided. See [Payment Retractions — Webhook Events](page:webhook-events/webhookevents-payment-retractions) for details.
...
This event's request payload is of type [PaymentRetractionVoidedWebhookRequest](../../../../doc/models/payment-retraction-voided-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentRetractionVoidedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentRetractionVoidedWebhookRequest) && event.event_type == 'PAYMENTRETRACTIONQUOTES.VOIDED'
      puts 'PaymentRetractionVoidedWebhook received'
      # TODO: Add paymentretractionvoidedwebhook handling
