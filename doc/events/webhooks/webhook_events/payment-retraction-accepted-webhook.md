...
# Payment Retraction Accepted Webhook

A payment retraction quote has been accepted. See [Payment Retractions — Webhook Events](page:webhook-events/webhookevents-payment-retractions) for details.
...
This event's request payload is of type [PaymentRetractionAcceptedWebhookRequest](../../../../doc/models/payment-retraction-accepted-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentRetractionAcceptedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentRetractionAcceptedWebhookRequest) && event.event_type == 'PAYMENTRETRACTIONQUOTES.ACCEPTED'
      puts 'PaymentRetractionAcceptedWebhook received'
      # TODO: Add paymentretractionacceptedwebhook handling
