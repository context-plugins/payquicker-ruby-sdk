...
A receipt has been created. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptCreatedWebhookRequest](../../../../doc/models/receipt-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptCreatedWebhookRequest) && event.event_type == 'RECEIPTS.CREATED'
      puts 'ReceiptCreatedWebhook received'
      # TODO: Add receiptcreatedwebhook handling
