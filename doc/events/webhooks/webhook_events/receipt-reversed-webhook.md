...
# Receipt Reversed Webhook

Receipt status changed to reversed. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptReversedWebhookRequest](../../../../doc/models/receipt-reversed-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptReversedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptReversedWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.STATUS.REVERSED'
      puts 'ReceiptReversedWebhook received'
      # TODO: Add receiptreversedwebhook handling
