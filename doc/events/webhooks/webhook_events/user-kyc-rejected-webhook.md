...
# User Kyc Rejected Webhook

User KYC has been rejected. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserKycRejectedWebhookRequest](../../../../doc/models/user-kyc-rejected-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserKycRejectedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserKycRejectedWebhookRequest) && event.event_type == 'USERS.UPDATED.KYC.REJECTED'
      puts 'UserKycRejectedWebhook received'
      # TODO: Add userkycrejectedwebhook handling
