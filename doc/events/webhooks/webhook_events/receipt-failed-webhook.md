...
# Receipt Failed Webhook

Receipt status changed to failed. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptFailedWebhookRequest](../../../../doc/models/receipt-failed-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptFailedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptFailedWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.STATUS.FAILED'
      puts 'ReceiptFailedWebhook received'
      # TODO: Add receiptfailedwebhook handling
