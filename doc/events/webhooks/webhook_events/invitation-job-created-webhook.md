...
# Invitation Job Created Webhook

A new batch invitation job has been created. See [Batch Invitations — Webhook Events](page:webhook-events/webhookevents-batch-invitations) for details.
...
This event's request payload is of type [InvitationJobCreatedWebhookRequest](../../../../doc/models/invitation-job-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `InvitationJobCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(InvitationJobCreatedWebhookRequest) && event.event_type == 'INVITATIONJOBS.CREATED'
      puts 'InvitationJobCreatedWebhook received'
      # TODO: Add invitationjobcreatedwebhook handling
