...
# Prepaid Card Pending Order Webhook

Card status changed to pending order. See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardPendingOrderWebhookRequest](../../../../doc/models/prepaid-card-pending-order-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardPendingOrderWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardPendingOrderWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.PENDINGORDER'
      puts 'PrepaidCardPendingOrderWebhook received'
      # TODO: Add prepaidcardpendingorderwebhook handling
