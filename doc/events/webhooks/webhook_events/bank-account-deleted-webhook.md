...
# Bank Account Deleted Webhook

Bank account status changed to deleted. See [Bank Accounts — Webhook Events](page:webhook-events/webhookevents-bank-accounts) for details.
...
This event's request payload is of type [BankAccountDeletedWebhookRequest](../../../../doc/models/bank-account-deleted-webhook-request.md).
...
```ruby
# Implementation example of handling the `BankAccountDeletedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(BankAccountDeletedWebhookRequest) && event.event_type == 'BANKACCOUNTS.UPDATED.STATUS.DELETED'
