...
# Invitation Job Failed Webhook

A batch invitation job has failed. See [Batch Invitations — Webhook Events](page:webhook-events/webhookevents-batch-invitations) for details.
...
This event's request payload is of type [InvitationJobFailedWebhookRequest](../../../../doc/models/invitation-job-failed-webhook-request.md).
...
```ruby
# Implementation example of handling the `InvitationJobFailedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(InvitationJobFailedWebhookRequest) && event.event_type == 'INVITATIONJOBS.FAILED'
      puts 'InvitationJobFailedWebhook received'
      # TODO: Add invitationjobfailedwebhook handling
