...
# User Kyc Approved Webhook

User KYC has been approved. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserKycApprovedWebhookRequest](../../../../doc/models/user-kyc-approved-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserKycApprovedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserKycApprovedWebhookRequest) && event.event_type == 'USERS.UPDATED.KYC.APPROVED'
      puts 'UserKycApprovedWebhook received'
      # TODO: Add userkycapprovedwebhook handling
