...
# Prepaid Card Closed Fraud Webhook

Card status changed to closed (fraud). See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardClosedFraudWebhookRequest](../../../../doc/models/prepaid-card-closed-fraud-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardClosedFraudWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardClosedFraudWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.CLOSEDFRAUD'
      puts 'PrepaidCardClosedFraudWebhook received'
      # TODO: Add prepaidcardclosedfraudwebhook handling
