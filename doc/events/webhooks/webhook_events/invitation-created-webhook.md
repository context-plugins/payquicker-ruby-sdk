...
# Invitation Created Webhook

A new invitation has been created. See [Invitations — Webhook Events](page:webhook-events/webhookevents-invitations) for details.
...
This event's request payload is of type [InvitationCreatedWebhookRequest](../../../../doc/models/invitation-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `InvitationCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(InvitationCreatedWebhookRequest) && event.event_type == 'INVITATIONS.CREATED'
      puts 'InvitationCreatedWebhook received'
      # TODO: Add invitationcreatedwebhook handling
