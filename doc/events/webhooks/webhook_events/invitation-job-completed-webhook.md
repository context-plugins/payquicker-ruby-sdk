...
# Invitation Job Completed Webhook

A batch invitation job has completed. See [Batch Invitations — Webhook Events](page:webhook-events/webhookevents-batch-invitations) for details.
...
This event's request payload is of type [InvitationJobCompletedWebhookRequest](../../../../doc/models/invitation-job-completed-webhook-request.md).
...
```ruby
# Implementation example of handling the `InvitationJobCompletedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(InvitationJobCompletedWebhookRequest) && event.event_type == 'INVITATIONJOBS.COMPLETED'
      puts 'InvitationJobCompletedWebhook received'
      # TODO: Add invitationjobcompletedwebhook handling
