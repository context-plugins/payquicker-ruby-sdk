...
# Payment Quote Created Webhook

A new payment quote has been created. See [Payments — Webhook Events](page:webhook-events/webhookevents-payments) for details.
...
This event's request payload is of type [PaymentQuoteCreatedWebhookRequest](../../../../doc/models/payment-quote-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentQuoteCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentQuoteCreatedWebhookRequest) && event.event_type == 'PAYMENTQUOTES.CREATED'
      puts 'PaymentQuoteCreatedWebhook received'
      # TODO: Add paymentquotecreatedwebhook handling
