...
# Payment Quote Accepted Webhook

A payment quote has been accepted. See [Payments — Webhook Events](page:webhook-events/webhookevents-payments) for details.
...
This event's request payload is of type [PaymentQuoteAcceptedWebhookRequest](../../../../doc/models/payment-quote-accepted-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentQuoteAcceptedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentQuoteAcceptedWebhookRequest) && event.event_type == 'PAYMENTQUOTES.ACCEPTED'
      puts 'PaymentQuoteAcceptedWebhook received'
      # TODO: Add paymentquoteacceptedwebhook handling
