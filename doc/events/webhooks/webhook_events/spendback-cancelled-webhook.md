...
# Spendback Cancelled Webhook

A spendback quote has been cancelled. See [Spendbacks — Webhook Events](page:webhook-events/webhookevents-spendbacks) for details.
...
This event's request payload is of type [SpendbackCancelledWebhookRequest](../../../../doc/models/spendback-cancelled-webhook-request.md).
...
```ruby
# Implementation example of handling the `SpendbackCancelledWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(SpendbackCancelledWebhookRequest) && event.event_type == 'SPENDBACKQUOTES.CANCELLED'
      puts 'SpendbackCancelledWebhook received'
      # TODO: Add spendbackcancelledwebhook handling
