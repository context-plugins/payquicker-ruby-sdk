...
# Prepaid Card Pending Activation Webhook

Card status changed to pending activation. See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardPendingActivationWebhookRequest](../../../../doc/models/prepaid-card-pending-activation-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardPendingActivationWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardPendingActivationWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.PENDINGACTIVATION'
      puts 'PrepaidCardPendingActivationWebhook received'
      # TODO: Add prepaidcardpendingactivationwebhook handling
