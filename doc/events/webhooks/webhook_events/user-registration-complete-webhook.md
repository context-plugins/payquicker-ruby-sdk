...
# User Registration Complete Webhook

User registration is complete. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserRegistrationCompleteWebhookRequest](../../../../doc/models/user-registration-complete-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserRegistrationCompleteWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserRegistrationCompleteWebhookRequest) && event.event_type == 'USERS.UPDATED.STATUS.REGISTRATIONCOMPLETE'
      puts 'UserRegistrationCompleteWebhook received'
      # TODO: Add userregistrationcompletewebhook handling
