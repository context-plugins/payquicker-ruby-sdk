...
# User Registration in Progress Webhook

User registration is in progress. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserRegistrationInProgressWebhookRequest](../../../../doc/models/user-registration-in-progress-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserRegistrationInProgressWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserRegistrationInProgressWebhookRequest) && event.event_type == 'USERS.UPDATED.STATUS.REGISTRATIONINPROGRESS'
      puts 'UserRegistrationInProgressWebhook received'
      # TODO: Add userregistrationinprogresswebhook handling
