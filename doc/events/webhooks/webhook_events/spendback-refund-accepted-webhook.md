...
# Spendback Refund Accepted Webhook

A spendback refund quote has been accepted. See [Spendback Refunds — Webhook Events](page:webhook-events/webhookevents-spendback-refunds) for details.
...
This event's request payload is of type [SpendbackRefundAcceptedWebhookRequest](../../../../doc/models/spendback-refund-accepted-webhook-request.md).
...
```ruby
# Implementation example of handling the `SpendbackRefundAcceptedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(SpendbackRefundAcceptedWebhookRequest) && event.event_type == 'SPENDBACKREFUNDQUOTES.ACCEPTED'
      puts 'SpendbackRefundAcceptedWebhook received'
      # TODO: Add spendbackrefundacceptedwebhook handling
