...
# User Event Started Webhook

A user event has started. See [User Events — Webhook Events](page:webhook-events/webhookevents-user-events) for details.
...
This event's request payload is of type [UserEventStartedWebhookRequest](../../../../doc/models/user-event-started-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserEventStartedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserEventStartedWebhookRequest) && event.event_type == 'USEREVENTS.STARTED'
      puts 'UserEventStartedWebhook received'
      # TODO: Add usereventstartedwebhook handling
