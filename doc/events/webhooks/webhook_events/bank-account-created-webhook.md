...
A new bank account has been created. See [Bank Accounts — Webhook Events](page:webhook-events/webhookevents-bank-accounts) for details.
...
This event's request payload is of type [BankAccountCreatedWebhookRequest](../../../../doc/models/bank-account-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `BankAccountCreatedWebhook` event (with signature verification) in Rails.
