...
# Receipt Refunded Webhook

Receipt status changed to refunded. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptRefundedWebhookRequest](../../../../doc/models/receipt-refunded-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptRefundedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptRefundedWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.STATUS.REFUNDED'
      puts 'ReceiptRefundedWebhook received'
      # TODO: Add receiptrefundedwebhook handling
