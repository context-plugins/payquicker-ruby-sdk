...
# Ewallet Created Webhook

An electronic wallet has been created. See [Electronic Wallets — Webhook Events](page:webhook-events/webhookevents-electronic-wallets) for details.
...
This event's request payload is of type [EwalletCreatedWebhookRequest](../../../../doc/models/ewallet-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `EwalletCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(EwalletCreatedWebhookRequest) && event.event_type == 'ELECTRONICWALLETS.CREATED'
      puts 'EwalletCreatedWebhook received'
      # TODO: Add ewalletcreatedwebhook handling
