...
# Payment Quote Cancelled Webhook

A payment quote has been cancelled. See [Payments — Webhook Events](page:webhook-events/webhookevents-payments) for details.
...
This event's request payload is of type [PaymentQuoteCancelledWebhookRequest](../../../../doc/models/payment-quote-cancelled-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentQuoteCancelledWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentQuoteCancelledWebhookRequest) && event.event_type == 'PAYMENTQUOTES.CANCELLED'
      puts 'PaymentQuoteCancelledWebhook received'
      # TODO: Add paymentquotecancelledwebhook handling
