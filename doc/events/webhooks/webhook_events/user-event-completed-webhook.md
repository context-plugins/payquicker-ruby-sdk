...
# User Event Completed Webhook

A user event has been completed. See [User Events — Webhook Events](page:webhook-events/webhookevents-user-events) for details.
...
This event's request payload is of type [UserEventCompletedWebhookRequest](../../../../doc/models/user-event-completed-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserEventCompletedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserEventCompletedWebhookRequest) && event.event_type == 'USEREVENTS.COMPLETED'
      puts 'UserEventCompletedWebhook received'
      # TODO: Add usereventcompletedwebhook handling
