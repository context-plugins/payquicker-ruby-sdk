...
# Prepaid Card Closed Expired Webhook

Card status changed to closed (expired). See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardClosedExpiredWebhookRequest](../../../../doc/models/prepaid-card-closed-expired-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardClosedExpiredWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardClosedExpiredWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.CLOSEDEXPIRED'
      puts 'PrepaidCardClosedExpiredWebhook received'
      # TODO: Add prepaidcardclosedexpiredwebhook handling
