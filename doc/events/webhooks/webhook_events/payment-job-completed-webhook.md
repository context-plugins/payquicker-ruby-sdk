...
# Payment Job Completed Webhook

A batch payment job has completed. See [Batch Payments — Webhook Events](page:webhook-events/webhookevents-batch-payments) for details.
...
This event's request payload is of type [PaymentJobCompletedWebhookRequest](../../../../doc/models/payment-job-completed-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentJobCompletedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentJobCompletedWebhookRequest) && event.event_type == 'PAYMENTJOBS.COMPLETED'
      puts 'PaymentJobCompletedWebhook received'
      # TODO: Add paymentjobcompletedwebhook handling
