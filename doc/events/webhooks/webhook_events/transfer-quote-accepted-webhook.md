...
# Transfer Quote Accepted Webhook

A transfer quote has been accepted. See [Transfers — Webhook Events](page:webhook-events/webhookevents-transfers) for details.
...
This event's request payload is of type [TransferQuoteAcceptedWebhookRequest](../../../../doc/models/transfer-quote-accepted-webhook-request.md).
...
```ruby
# Implementation example of handling the `TransferQuoteAcceptedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(TransferQuoteAcceptedWebhookRequest) && event.event_type == 'TRANSFERQUOTES.ACCEPTED'
      puts 'TransferQuoteAcceptedWebhook received'
      # TODO: Add transferquoteacceptedwebhook handling
