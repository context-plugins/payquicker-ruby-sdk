...
# Prepaid Card Closed Webhook

Card status changed to closed. See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardClosedWebhookRequest](../../../../doc/models/prepaid-card-closed-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardClosedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardClosedWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.CLOSED'
      puts 'PrepaidCardClosedWebhook received'
      # TODO: Add prepaidcardclosedwebhook handling
