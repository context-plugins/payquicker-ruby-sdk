...
# User Event Cancelled Webhook

A user event has been cancelled. See [User Events — Webhook Events](page:webhook-events/webhookevents-user-events) for details.
...
This event's request payload is of type [UserEventCancelledWebhookRequest](../../../../doc/models/user-event-cancelled-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserEventCancelledWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserEventCancelledWebhookRequest) && event.event_type == 'USEREVENTS.CANCELLED'
      puts 'UserEventCancelledWebhook received'
      # TODO: Add usereventcancelledwebhook handling
