...
# Transfer Quote Voided Webhook

A transfer quote has been voided. See [Transfers — Webhook Events](page:webhook-events/webhookevents-transfers) for details.
...
This event's request payload is of type [TransferQuoteVoidedWebhookRequest](../../../../doc/models/transfer-quote-voided-webhook-request.md).
...
```ruby
# Implementation example of handling the `TransferQuoteVoidedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(TransferQuoteVoidedWebhookRequest) && event.event_type == 'TRANSFERQUOTES.VOIDED'
      puts 'TransferQuoteVoidedWebhook received'
      # TODO: Add transferquotevoidedwebhook handling
