...
# Receipt Modified Webhook

Receipt details have been modified. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptModifiedWebhookRequest](../../../../doc/models/receipt-modified-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptModifiedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptModifiedWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.DETAILS.MODIFIED'
      puts 'ReceiptModifiedWebhook received'
      # TODO: Add receiptmodifiedwebhook handling
