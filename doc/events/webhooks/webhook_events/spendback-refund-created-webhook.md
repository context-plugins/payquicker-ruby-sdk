...
# Spendback Refund Created Webhook

A new spendback refund quote has been created. See [Spendback Refunds — Webhook Events](page:webhook-events/webhookevents-spendback-refunds) for details.
...
This event's request payload is of type [SpendbackRefundCreatedWebhookRequest](../../../../doc/models/spendback-refund-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `SpendbackRefundCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(SpendbackRefundCreatedWebhookRequest) && event.event_type == 'SPENDBACKREFUNDQUOTES.CREATED'
      puts 'SpendbackRefundCreatedWebhook received'
      # TODO: Add spendbackrefundcreatedwebhook handling
