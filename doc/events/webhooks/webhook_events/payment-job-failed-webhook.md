...
# Payment Job Failed Webhook

A batch payment job has failed. See [Batch Payments — Webhook Events](page:webhook-events/webhookevents-batch-payments) for details.
...
This event's request payload is of type [PaymentJobFailedWebhookRequest](../../../../doc/models/payment-job-failed-webhook-request.md).
...
```ruby
# Implementation example of handling the `PaymentJobFailedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PaymentJobFailedWebhookRequest) && event.event_type == 'PAYMENTJOBS.FAILED'
      puts 'PaymentJobFailedWebhook received'
      # TODO: Add paymentjobfailedwebhook handling
