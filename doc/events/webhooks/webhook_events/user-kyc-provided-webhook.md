...
# User Kyc Provided Webhook

User KYC documentation has been provided. See [Users — Webhook Events](page:webhook-events/webhookevents-users) for details.
...
This event's request payload is of type [UserKycProvidedWebhookRequest](../../../../doc/models/user-kyc-provided-webhook-request.md).
...
```ruby
# Implementation example of handling the `UserKycProvidedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(UserKycProvidedWebhookRequest) && event.event_type == 'USERS.UPDATED.KYC.PROVIDED'
      puts 'UserKycProvidedWebhook received'
      # TODO: Add userkycprovidedwebhook handling
