...
# Ewallet Modified Webhook

Ewallet details have been modified. See [Electronic Wallets — Webhook Events](page:webhook-events/webhookevents-electronic-wallets) for details.
...
This event's request payload is of type [EwalletModifiedWebhookRequest](../../../../doc/models/ewallet-modified-webhook-request.md).
...
```ruby
# Implementation example of handling the `EwalletModifiedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(EwalletModifiedWebhookRequest) && event.event_type == 'ELECTRONICWALLETS.UPDATED.DETAILS.MODIFIED'
      puts 'EwalletModifiedWebhook received'
      # TODO: Add ewalletmodifiedwebhook handling
