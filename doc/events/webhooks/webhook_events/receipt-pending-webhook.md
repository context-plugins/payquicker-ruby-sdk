...
# Receipt Pending Webhook

Receipt status changed to pending. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptPendingWebhookRequest](../../../../doc/models/receipt-pending-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptPendingWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptPendingWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.STATUS.PENDING'
      puts 'ReceiptPendingWebhook received'
      # TODO: Add receiptpendingwebhook handling
