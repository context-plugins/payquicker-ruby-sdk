...
# Ewallet Deleted Webhook

Ewallet status changed to deleted. See [Electronic Wallets — Webhook Events](page:webhook-events/webhookevents-electronic-wallets) for details.
...
This event's request payload is of type [EwalletDeletedWebhookRequest](../../../../doc/models/ewallet-deleted-webhook-request.md).
...
```ruby
# Implementation example of handling the `EwalletDeletedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(EwalletDeletedWebhookRequest) && event.event_type == 'ELECTRONICWALLETS.UPDATED.STATUS.DELETED'
      puts 'EwalletDeletedWebhook received'
      # TODO: Add ewalletdeletedwebhook handling
