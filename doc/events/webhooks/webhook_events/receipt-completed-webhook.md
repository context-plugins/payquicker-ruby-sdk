...
# Receipt Completed Webhook

Receipt status changed to completed. See [Receipts — Webhook Events](page:webhook-events/webhookevents-receipts) for details.
...
This event's request payload is of type [ReceiptCompletedWebhookRequest](../../../../doc/models/receipt-completed-webhook-request.md).
...
```ruby
# Implementation example of handling the `ReceiptCompletedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(ReceiptCompletedWebhookRequest) && event.event_type == 'RECEIPTS.UPDATED.STATUS.COMPLETED'
      puts 'ReceiptCompletedWebhook received'
      # TODO: Add receiptcompletedwebhook handling
