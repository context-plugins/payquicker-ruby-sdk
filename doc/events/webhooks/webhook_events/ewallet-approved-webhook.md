...
# Ewallet Approved Webhook

Ewallet status changed to approved. See [Electronic Wallets — Webhook Events](page:webhook-events/webhookevents-electronic-wallets) for details.
...
This event's request payload is of type [EwalletApprovedWebhookRequest](../../../../doc/models/ewallet-approved-webhook-request.md).
...
```ruby
# Implementation example of handling the `EwalletApprovedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(EwalletApprovedWebhookRequest) && event.event_type == 'ELECTRONICWALLETS.UPDATED.STATUS.APPROVED'
      puts 'EwalletApprovedWebhook received'
      # TODO: Add ewalletapprovedwebhook handling
