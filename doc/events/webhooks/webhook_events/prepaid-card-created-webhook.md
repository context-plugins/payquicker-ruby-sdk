...
# Prepaid Card Created Webhook

A prepaid card has been created. See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardCreatedWebhookRequest](../../../../doc/models/prepaid-card-created-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardCreatedWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardCreatedWebhookRequest) && event.event_type == 'PREPAIDCARDS.CREATED'
      puts 'PrepaidCardCreatedWebhook received'
      # TODO: Add prepaidcardcreatedwebhook handling
