...
# Prepaid Card Closed Lost Webhook

Card status changed to closed (lost). See [Prepaid Cards — Webhook Events](page:webhook-events/webhookevents-prepaid-cards) for details.
...
This event's request payload is of type [PrepaidCardClosedLostWebhookRequest](../../../../doc/models/prepaid-card-closed-lost-webhook-request.md).
...
```ruby
# Implementation example of handling the `PrepaidCardClosedLostWebhook` event (with signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(PrepaidCardClosedLostWebhookRequest) && event.event_type == 'PREPAIDCARDS.UPDATED.STATUS.CLOSEDLOST'
      puts 'PrepaidCardClosedLostWebhook received'
      # TODO: Add prepaidcardclosedlostwebhook handling
